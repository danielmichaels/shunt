...
func printCheatsheet(httpAddr string, natsPort int) {
	...
	fmt.Fprintf(os.Stderr, "\n  Subscribe to routed messages (covers the bundled examples, minus JS noise):\n")
	fmt.Fprintf(os.Stderr, "    nats -s nats://localhost:%d sub \\\n", natsPort)
	fmt.Fprintf(os.Stderr, "      'alerts.>' 'facilities.>' 'monitoring.>' 'escalation.>' 'ops.>' \\\n")
	fmt.Fprintf(os.Stderr, "      'critical.>' 'github.>' 'webhooks.>' 'payments.>' 'tenants.>'\n")
	fmt.Fprintf(os.Stderr, "    # examples above land in alerts.>, facilities.>, critical.>, github.>, webhooks.>\n")
	fmt.Fprintf(os.Stderr, "    # avoid nats sub '>' unless you want JetStream/$JS chatter too\n")
	fmt.Fprintf(os.Stderr, "\n  ────────────────────────────────────────────────────────────\n\n")
}
